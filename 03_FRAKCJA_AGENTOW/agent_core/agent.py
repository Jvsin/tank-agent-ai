...
from .checkpoints import (
    build_checkpoints_to_enemy,
    get_firing_range,
    lane_offset_checkpoint,
)
...
class SmartAgent:
    ...
    def _terrain_damage(terrain: Any) -> float:
        if isinstance(terrain, dict):
            return float(
                terrain.get(
                    "deal_damage", terrain.get("_deal_damage", terrain.get("dmg", 0))
                )
                or 0
            )
        return float(
            getattr(terrain, "deal_damage", getattr(terrain, "_deal_damage", 0)) or 0
        )
    ...
    def _powerup_type_text(powerup: Any) -> str:
        if isinstance(powerup, dict):
            return str(
                powerup.get("powerup_type", powerup.get("_powerup_type", ""))
            ).lower()
        return str(
            getattr(powerup, "powerup_type", getattr(powerup, "_powerup_type", ""))
        ).lower()

    def _mark_allies_occupancy(
        self, my_x: float, my_y: float, seen_allies: List[Any]
    ) -> None:
        my_cell = self.model.to_cell(my_x, my_y)
        for ally in seen_allies:
            pos = (
                ally.get("position", ally.get("_position", {}))
                if isinstance(ally, dict)
                else getattr(ally, "position", getattr(ally, "_position", None))
            )
            ax, ay = self._xy(pos)
            ...
            heading = float(
                ally.get("heading", ally.get("_heading", 0.0))
                if isinstance(ally, dict)
                else getattr(ally, "heading", getattr(ally, "_heading", 0.0)) or 0.0
            )
    ...
    def _mark_enemy_occupancy(
        self, my_x: float, my_y: float, seen_enemies: List[Any]
    ) -> None:
        my_cell = self.model.to_cell(my_x, my_y)
        for enemy in seen_enemies:
            pos = (
                enemy.get("position", enemy.get("_position", {}))
                if isinstance(enemy, dict)
                else getattr(enemy, "position", getattr(enemy, "_position", None))
            )
            ex, ey = self._xy(pos)
            ...
            heading = float(
                enemy.get("heading", enemy.get("_heading", 0.0))
                if isinstance(enemy, dict)
                else getattr(enemy, "heading", getattr(enemy, "_heading", 0.0)) or 0.0
            )
            heading_rad = math.radians(heading)
            for step, ttl in ((1, 4.0), (2, 2.5)):
    ...
    def _update_world(
        ...
        for obstacle in sensor.get("seen_obstacles", []):
            pos = (
                obstacle.get("position", obstacle.get("_position", {}))
                if isinstance(obstacle, dict)
                else getattr(obstacle, "position", getattr(obstacle, "_position", None))
            )
            ox, oy = self._xy(pos)
        ...
        for terrain in sensor.get("seen_terrains", []):
            pos = (
                terrain.get("position", terrain.get("_position", {}))
                if isinstance(terrain, dict)
                else getattr(terrain, "position", getattr(terrain, "_position", None))
            )
            tx, ty = self._xy(pos)
            cell = self.model.to_cell(tx, ty)
            damage = self._terrain_damage(terrain)
            terrain_type = (
                str(terrain.get("type", "")).lower()
                if isinstance(terrain, dict)
                else str(
                    getattr(
                        terrain, "terrain_type", getattr(terrain, "_terrain_type", "")
                    )
                ).lower()
            )
        ...
        for powerup in sensor.get("seen_powerups", []):
            pos = (
                powerup.get("position", powerup.get("_position", {}))
                if isinstance(powerup, dict)
                else getattr(powerup, "position", getattr(powerup, "_position", None))
            )
            px, py = self._xy(pos)
            ...
            if "medkit" in ptxt and hp_ratio <= 0.75:
                ...
                is_preferred = (
                    wants_primary or ammo_stocks.get(self.preferred_ammo, 0) <= 0
                )

            if is_preferred:
                self.model.preferred_powerup_cells.add(powerup_cell)

    def _standing_on_danger(
        self, my_x: float, my_y: float, sensor: Dict[str, Any]
    ) -> bool:
    ...
    def _enemy_in_firing_range(
        ...
        for tank in sensor.get("seen_tanks", []):
            pos = (
                tank.get("position", tank.get("_position", {}))
                if isinstance(tank, dict)
                else getattr(tank, "position", getattr(tank, "_position", None))
            )
            tx, ty = self._xy(pos)
    ...
    def _ally_blocking_front(
        self, my_x: float, my_y: float, my_heading: float, allies: List[Any]
    ) -> bool:
        return self._blocking_ally_id(my_x, my_y, my_heading, allies) is not None

    def _blocking_ally_id(
        self, my_x: float, my_y: float, my_heading: float, allies: List[Any]
    ) -> Optional[str]:
        for ally in allies:
            pos = (
    ...
    def _enemy_blocking_front(
        self, my_x: float, my_y: float, my_heading: float, enemies: List[Any]
    ) -> bool:
    ...
    def _lane_adjusted_checkpoint(
        self, tank_id: str, checkpoint: Tuple[float, float]
    ) -> Tuple[float, float]:
    ...
    def _choose_ammo_to_load(
        self, my_tank_status: Dict[str, Any], enemy_close: bool
    ) -> Optional[str]:
    ...
    def _closest_destructible_obstacle(
        ...
        for obs in sensor.get("seen_obstacles", []):
            if not (
                obs.get("is_destructible", False)
                if isinstance(obs, dict)
                else getattr(obs, "is_destructible", False)
            ):
                continue
            pos = (
                obs.get("position", obs.get("_position", {}))
                if isinstance(obs, dict)
                else getattr(obs, "position", getattr(obs, "_position", None))
            )
            ox, oy = self._xy(pos)
    ...
    def _update_path(self, my_x: float, my_y: float, goal: Goal, tick: int) -> None:
        ...
        need_replan = (
            ...
            or self.current_goal.cell != goal.cell
            or tick >= self.replan_tick
            or not self.driver.path
        )
    ...
    def _escape_target_cell(
        self, my_cell: Tuple[int, int]
    ) -> Optional[Tuple[int, int]]:
        for radius, require_known in ((10, True), (14, True), (18, False), (24, False)):
            safe_cell = self.goal_selector.nearest_safe_cell(
                my_cell, radius=radius, require_known=require_known
            )
    ...
    def _important_sensor_event(
        ...
        *,
        ...
        if enemies_visible:
            ENGAGE_VISIBLE_DISTANCE = 6.0
            close_enemies = 0
            for t in sensor.get("seen_tanks", []):
                tx, ty = (
                    self._xy(t.get("position", t.get("_position", {})))
                    if isinstance(t, dict)
                    else (getattr(t, "position").x, getattr(t, "position").y)
                )
    ...
    def _local_probe_goal(self, my_cell: Tuple[int, int]) -> Optional[Goal]:
        ...
        if best_cell is None:
            fallback = self.driver.best_immediate_safe_neighbor(
                my_cell, allow_risky=True
            )
            if fallback is None:
                return None
            best_cell = fallback
    ...
    def _movement_risk_score(
        ...
        for obstacle in sensor.get("seen_obstacles", []):
            ...
            if rel > cone_width:
                continue
            risk += max(0.0, (max_probe - dist) * 0.33) + max(
                0.0, (cone_width - rel) * 0.03
            )
    ...
    def _self_preserving_command(
        ...
        base_risk = self._movement_risk_score(
            my_x, my_y, my_heading, base_turn, base_speed, sensor
        )
        ...
        for t_off in turn_offsets:
            ...
            for scale in speed_scales:
                ...
                risk = self._movement_risk_score(
                    my_x, my_y, my_heading, candidate_turn, candidate_speed, sensor
                )
                score = (
                    risk
    ...
    def _stabilize_direction_and_speed(
        ...
        *,
        ...
        max_turn_delta = 3.6
        turn = previous_turn + self._clamp(
            turn - previous_turn, -max_turn_delta, max_turn_delta
        )
        ...
        if sign_changed:
            if (
                current_tick - self.last_turn_flip_tick < 58
                and abs(turn) < 11.0
                and abs(previous_turn) > 2.5
            ):
        ...
        cruise_mode = (
            mode.startswith("route_commit")
            or mode.startswith("checkpoint")
            or mode in ("control_lane", "explore", "local_probe", "patrol")
        )
    ...
    def get_action(
        ...
        if my_team is not None:
            seen_allies = [
                ...
                and str(tank.get("id", tank.get("_id", ""))) != self.my_tank_id
            ]
            sensor["seen_tanks"] = [
                tank
                for tank in seen_tanks
        ...
        ally_blocking_front = self._ally_blocking_front(
            my_x, my_y, my_heading, seen_allies
        )
        enemy_blocking_front = self._enemy_blocking_front(
            my_x, my_y, my_heading, sensor.get("seen_tanks", [])
        )
        enemy_close = self._closest_enemy(my_x, my_y, sensor)
        ammo_to_load = self._choose_ammo_to_load(
            my_tank_status, enemy_close is not None and enemy_close[2] <= 22.0
        )
        ...
        stuck_triggered = self.driver.update_stuck(
            ...
            enemies_visible,
            my_heading,
            blocking_tank_in_front=blocking_tank_in_front,
        )
        ...
        standing_on_danger = live_hazard_now or self._standing_on_danger(
            my_x, my_y, sensor
        )
        ...
        obstacle_avoid = self._reactive_obstacle_avoidance(
            my_x, my_y, my_heading, top_speed, sensor, seen_allies=seen_allies
        )

        if live_hazard_now:
            safe_patch = self._closest_non_hazard_point(my_x, my_y, sensor)
            if safe_patch is not None and safe_patch[2] <= 34.0:
                turn, speed = self.driver.drive_to_point(
                    my_x, my_y, my_heading, safe_patch[0], safe_patch[1], top_speed
                )
            elif obstacle_avoid is not None:
                turn, speed = obstacle_avoid
            else:
                push_cell = self.driver.best_immediate_safe_neighbor(
                    my_cell, allow_risky=True
                )
                if push_cell is not None and push_cell != my_cell:
                    turn, speed = self.driver.drive_to_cell(
                        my_x, my_y, my_heading, push_cell, top_speed
                    )
                else:
                    turn, speed = self.driver.escape_drive(
                        my_x, my_y, my_heading, top_speed
                    )
            ...
            if escape_goal_cell is not None and escape_goal_cell != self.model.to_cell(
                my_x, my_y
            ):
                self.current_goal = Goal(escape_goal_cell, "escape_route", 999.0)
                if (
                    not self.driver.path
                    or (self.driver.path and self.driver.path[-1] != escape_goal_cell)
                    or current_tick >= self.replan_tick
                ):
                    self.driver.path = self.planner.build_path(
                        my_cell, escape_goal_cell, radius=26
                    )
            ...
            if stuck_triggered and (ally_blocking_front or enemy_blocking_front):
                if ally_blocking_front:
                    blocking_id = self._blocking_ally_id(
                        my_x, my_y, my_heading, seen_allies
                    )
            ...
            enemy_in_range = self._enemy_in_firing_range(
                my_x, my_y, sensor, firing_range
            )
            closest_enemy = self._closest_enemy(my_x, my_y, sensor)

            if self.driver.unblock_ticks > 0:
                turn, speed = self.driver.unblock_drive(
                    top_speed, add_turn=enemy_blocking_front
                )
                mode = "unblock"
                self.driver.path = []
                self.current_goal = None
                ...
                turn, _ = self.driver.drive_to_point(
                    my_x, my_y, my_heading, ex, ey, top_speed
                )
                ...
                if need_replan:
                    self.driver.path = self.planner.build_path(
                        my_cell, enemy_goal_cell, radius=30
                    )
                    self.replan_tick = current_tick + 8
                ...
                while self.driver.path:
                    ...
                    if euclidean_distance(my_x, my_y, wx, wy) < 2.5:
                        ...
                        break
                if self.driver.path:
                    turn, speed = self.driver.drive_path(
                        my_x, my_y, my_heading, top_speed
                    )
                else:
                    turn, speed = self.driver.drive_to_point(
                ...
                if idx < 0 and checkpoints:
                    idx = min(
                        ...
                        key=lambda i: euclidean_distance(
                            my_x, my_y, checkpoints[i][0], checkpoints[i][1]
                        ),
                    )
                ...
                while idx < len(checkpoints):
                    ...
                    if (
                        euclidean_distance(my_x, my_y, cx, cy)
                        < self.checkpoint_arrival_radius
                    ):
                        idx += 1
                        self.checkpoint_index_by_tank[tank_id] = idx
                    else:
                ...
                if idx < len(checkpoints):
                    ...
                    for i in range(idx, min(idx + 3, len(checkpoints))):
                        cxi, cyi = self._lane_adjusted_checkpoint(
                            tank_id, checkpoints[i]
                        )
                    ...
                    if self.driver.path:
                        ...
                        turn, speed = self.driver.drive_to_point(
                            my_x, my_y, my_heading, cx, cy, top_speed
                        )
        ...
        barrel_rotation, should_fire, _ = self.turret.update(
            ...
            current_ammo=str(my_tank_status.get("ammo_loaded", "") or "").upper()
            or None,
        ...
        destructible = self._closest_destructible_obstacle(
            my_x, my_y, sensor, max_dist=firing_range
        )
