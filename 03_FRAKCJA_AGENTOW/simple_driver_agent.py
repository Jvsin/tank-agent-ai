...
class SimpleDriverAgent:
    ...
    def _init_checkpoints(self, my_tank_status: Dict[str, Any], x: float) -> None:
        ...
        team = my_tank_status.get("_team")
        if team is None:
            team = 2 if x > 100.0 else 1
        ...
        print(f"[{self.name}] team={team} start_cp=0/{len(self.checkpoints)}")
    ...
    def get_action(
        ...
        if current_tick % 60 == 0:
            ...
            print(
                ...
                f"dist={dist:.1f} speed={speed:.2f} turn={turn:.1f}"
            )
