...
try:
    ...
    print(
        "Upewnij się, że skrypt jest uruchamiany z katalogu '02_FRAKCJA_SILNIKA' lub że struktura projektu jest poprawna."
    )
    sys.exit(1)
...
TEAM_B_SPAWN_POINTS = [
    ...
    (185, 165),
    (185, 195),
]  # Współrzędne (x, y) na Grass dla drużyny B
TARGET_FPS = 60
...
def generate_radial_explosion(
    particles_list: List[ExplosionParticle], position: tuple, num_particles: int
):
...
def generate_cone_explosion(
    particles_list: List[ExplosionParticle],
    position: tuple,
    num_particles: int,
    base_direction_vector: Vector2,
    cone_angle: float,
):
...
def load_assets():
    ...
    for name in tile_names:
        try:
            ...
            assets["tiles"][name] = pygame.transform.scale(
                img, (TILE_SIZE * SCALE, TILE_SIZE * SCALE)
            )
        except pygame.error:
        ...
        try:
            ...
            assets["tanks"][tank_type] = {
                "body": pygame.transform.scale(
                    pygame.image.load(
                        os.path.join(base_path, "tnk1.png")
                    ).convert_alpha(),
                    tank_render_size,
                ),
                "mask_body": pygame.transform.scale(
                    pygame.image.load(
                        os.path.join(base_path, "msk1.png")
                    ).convert_alpha(),
                    tank_render_size,
                ),
                "turret": pygame.transform.scale(
                    pygame.image.load(
                        os.path.join(base_path, "tnk2.png")
                    ).convert_alpha(),
                    tank_render_size,
                ),
                "mask_turret": pygame.transform.scale(
                    pygame.image.load(
                        os.path.join(base_path, "msk2.png")
                    ).convert_alpha(),
                    tank_render_size,
                ),
            }
    ...
    for tank_type, folder_name in TANK_ASSET_MAP.items():
        ...
        try:
            ...
            assets["icons"][tank_type] = pygame.Surface(
                icon_render_size, pygame.SRCALPHA
            )
...
def draw_tank(
    surface: pygame.Surface, tank: Tank, assets: Dict, scale: int, map_height: int
):
    ...
    if is_alive:
        ...
        turret_color_layer.blit(
            mask_turret_img, (0, 0), special_flags=pygame.BLEND_RGB_MULT
        )
        turret_color_layer.set_colorkey((0, 0, 0))
        rotated_turret_mask = pygame.transform.rotate(
            turret_color_layer, -total_turret_angle - 180
        )
    ...
    if is_alive:
        ...
        pygame.draw.rect(
            surface, (50, 50, 50), (hp_bar_x, hp_bar_y, hp_bar_width, hp_bar_height)
        )
        pygame.draw.rect(
            surface,
            (0, 255, 0),
            (hp_bar_x, hp_bar_y, hp_bar_width * hp_ratio, hp_bar_height),
        )
...
def create_background_surface(
    map_info: Any, assets: Dict, scale: int, width: int, height: int
) -> pygame.Surface:
...
def draw_ui(
    ...
    # Statystyki drużyn
    team1_tanks = sorted(
        [t for t in game_loop.tanks.values() if t.team == 1], key=lambda t: t._id
    )
    team1_alive = sum(1 for t in team1_tanks if t.is_alive())
    team1_kills = sum(
        s.tanks_killed for s in game_loop.scoreboards.values() if s.team == 1
    )

    team2_tanks = sorted(
        [t for t in game_loop.tanks.values() if t.team == 2], key=lambda t: t._id
    )
    team2_alive = sum(1 for t in team2_tanks if t.is_alive())
    team2_kills = sum(
        s.tanks_killed for s in game_loop.scoreboards.values() if s.team == 2
    )
    ...
    for tank in team1_tanks:
        ...
        bar_y = current_y
        pygame.draw.rect(
            screen, (100, 0, 0), (bar_x, bar_y, hp_bar_width, hp_bar_height)
        )
        pygame.draw.rect(
            screen,
            TEAM_COLORS[1],
            (bar_x, bar_y, hp_bar_width * hp_ratio, hp_bar_height),
        )
        pygame.draw.rect(
            screen, (255, 255, 255), (bar_x, bar_y, hp_bar_width, hp_bar_height), 1
        )
...
def main():
    ...
    # --- Inicjalizacja Gry ---
    game_loop = GameLoop(
        headless=False, spawn_points={1: TEAM_A_SPAWN_POINTS, 2: TEAM_B_SPAWN_POINTS}
    )
    ...
    try:
        ...
        background_surface = create_background_surface(
            game_loop.map_info, assets, SCALE, map_render_width, map_render_height
        )
        ...
        if game_loop.tanks:
            ...
            for tank in sorted_tanks:
                print(
                    f"  - Czołg: {tank._id} (Team: {tank.team}, Typ: {tank._tank_type}) na pozycji ({tank.position.x:.1f}, {tank.position.y:.1f})"
                )
        else:
            print("  Brak czołgów.")
        ...
        if game_loop.map_info and game_loop.map_info.powerup_list:
            for powerup in game_loop.map_info.powerup_list:
                print(
                    f"  - Power-up: {powerup.powerup_type.name} na pozycji ({powerup.position.x:.1f}, {powerup.position.y:.1f})"
                )
        else:
        ...
        while waiting_for_start:
            ...
            map_surface.blit(
                background_surface, (0, 0)
            )  # Narysuj tło na powierzchni mapy
            screen.blit(map_surface, map_rect)  # Narysuj powierzchnię mapy na ekranie
            draw_ui(
                screen, font, game_loop, window_width, map_rect, assets
            )  # Narysuj UI
            ...
            start_text_surf = start_font.render(
        ...
        while running:
            ...
            for hit in physics_results.get("projectile_hits", []):
                ...
                hit_screen_pos = None
                if hit.hit_position:
                    hit_screen_pos = Vector2(
                        hit.hit_position.x * SCALE,
                        map_render_height - (hit.hit_position.y * SCALE),
                    )
                ...
                if shooter_tank:
                    ...
                    final_turret_angle = (
                        shooter_tank.heading - shooter_tank.barrel_angle
                    )
                    ...
                    # Pozycja końca lufy
                    tank_center_pos = Vector2(
                        shooter_tank.position.x * SCALE,
                        map_render_height - (shooter_tank.position.y * SCALE),
                    )
                    barrel_length = (
                        TILE_SIZE * SCALE
                    ) * 0.8  # Długość lufy jako przybliżenie
                    barrel_tip_pos = (
                        tank_center_pos + visual_barrel_direction * barrel_length
                    )
                ...
                if hit_screen_pos:
                    generate_radial_explosion(
                        particles_list=explosion_particles,
                        position=hit_screen_pos,
                        num_particles=50,
                    )
            ...
            if destroyed_obstacle_ids:
                ...
                if grass_asset:
                    ...
                    for obstacle in game_loop.map_info.obstacle_list:
                        ...
                        if (
                            obstacle._id in destroyed_obstacle_ids
                            and not obstacle.is_alive
                        ):
                            ...
                            top_left = (
                                pos_x - grass_asset.get_width() / 2,
                                pos_y - grass_asset.get_height() / 2,
                            )
            ...
            for powerup in game_loop.map_info.powerup_list:
                ...
                if asset:
                    ...
                    top_left = (
                        pos_x - asset.get_width() / 2,
                        pos_y - asset.get_height() / 2,
                    )
        ...
        if scoreboards:
            scoreboards.sort(
                key=lambda x: (x.get("team", 0), -x.get("tanks_killed", 0))
            )
            for score in scoreboards:
                print(
                    f"  - Czołg: {score.get('tank_id')}, Drużyna: {score.get('team')}, "
